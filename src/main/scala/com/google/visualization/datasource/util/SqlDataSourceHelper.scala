...
object SqlDataSourceHelper {
  ...
  def executeQuery(query: Query, dbDescription: SqlDatabaseDescription, tableName: String): DataTable = {
    ...
    def getColumnIds(qs: QuerySelection) = qs.getColumns.asScala map (_.getId)
...
//    val columnIds = query.selectionOpt map getColumnIds getOrElse Seq.empty
  ...
  class SqlBuilder(query: Query, tableName: String) {
    ...
    def quoteCol(str: String): String = colQuoteChar +: str :+ colQuoteChar
    def quote(str: String): String = strQuoteChar +: str :+ strQuoteChar
    def unquote(str: String): String = str.filter(_ != strQuoteChar)
    ...
    def selectClause =
      query.selectionOpt map {
        _.getColumns.asScala map getColumnId mkString("SELECT ", ", ", " ")
      } getOrElse "SELECT * "
    ...
    def groupByClause =
      query.groupOpt map {
        _.getColumnIds.asScala map quoteCol mkString("GROUP BY ", ", ", " ")
      } getOrElse ""
    ...
    def orderByClause =
      query.sortOpt map { sort =>
        ...
        sort.getSortColumns.asScala map idWithOrdering mkString("ORDER BY ", ", ", " ")
      } getOrElse ""
      ...
      queryFilter match {
        ...
        case filter: CompoundFilter =>
          ...
          if (getSubFilters.isEmpty) {
            ...
            getSubFilters.asScala map buildWhereClauseFor mkString("(", logicalOperator, ")")
          }
      }
    }
