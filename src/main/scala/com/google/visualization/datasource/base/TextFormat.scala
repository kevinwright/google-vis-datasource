 ...
 * @author Hillel M.
 */
class TextFormat extends UFormat {
  /**
   ...
   */
  def format(obj: AnyRef, appendTo: StringBuffer, pos: FieldPosition) = {
    if ((null == obj) || !(obj.isInstanceOf[String])) {
      throw new IllegalArgumentException
    }
    val text = obj.asInstanceOf[String]
    appendTo append text
    pos setBeginIndex 0
    pos setEndIndex {if (0 == text.length) 0 else (text.length - 1) }
    appendTo
  }
